...
import { format, addDays, startOfWeek, isSameDay, getHours, getMinutes, differenceInMinutes } from 'date-fns';
        ...
        <div className="flex flex-col h-[calc(100vh-200px)] border border-border rounded-lg bg-card overflow-hidden">
            ...
            <div className="flex border-b border-border bg-secondary/50">
                ...
                <div className="flex-1 grid grid-cols-7 divide-x divide-border">
                    {days.map((day) => (
                        <div key={day.toString()} className={cn("p-2 text-center", isSameDay(day, new Date()) && "bg-indigo-50 dark:bg-indigo-900/20")}>
                            <div className="text-xs text-muted-foreground font-medium uppercase">{format(day, 'EEE')}</div>
                            <div className={cn("text-xl font-bold font-outfit mt-1", isSameDay(day, new Date()) && "text-primary")}>
                                {format(day, 'd')}
                            </div>
                        </div>
                    ))}
